...
define(['react', "react-redux", 
	...
	class ChatBox extends React.Component {
		...
		render(){
			return(
				<div className="mui-content mui-scroll-wrapper chatbox">
					<div id='msg-list' className="mui-scroll">
						<dl className="mui-table-view mui-table-view-chevron">
					{this.props.list.map(function(item1, index1){
						return (
							...
							{item1.list.map(function(item, index){
								return (
								<dd key={index} className={"mui-table-view-cell msg-item "+(item.fuid==this.props.uid?"msg-item-self":"")} data-msg-type={item.content_type} data-msg-content={item.content}>
									{item.fuid==this.props.uid ?
										<i className="msg-user mui-icon mui-icon-person"></i>
									:
										<img className="msg-user-img" src={this.state.talk_avatar} alt="" />
									}
									<div className="msg-content">
										<div className="msg-content-inner">
											{
												item.content_type == '文字'?
												<span dangerouslySetInnerHTML={{__html: this.replace_em(item.content)}}></span>
											:
												(
													item.content_type == '图片'?
													(item.content.substr(0,4)==="file"?
													<Img src={item.content} folder="chat" placeholder={item.content} data-loaded="true" />
													:
													<Img src={item.content} folder="chat" placeholder="./js/containers/img/120_90.png" />
													)
													
													:
													<span>
														<span className="mui-icon mui-icon-mic" style={{fontSize: "18px", fontWeight: "bold"}}></span>
														<span className="play-state">点击播放</span>
													</span>
												)
											}
										</div>
										<div className="msg-content-arrow"></div>
									</div>
									<div className="mui-item-clear"></div>
								</dd>	
								)
							}.bind(this))}
							</div>
