			...
			return ( 
				<div className="post_detail">
					<div className="company">
						<Header>
							...
							<h1 className="mui-title">帖子详情</h1>
						{mui.os.plus?
				...
				{!this.props.is_hide_comment?
					<div className="comments">
						<div className="mui-scroll-wrapper" id="refreshContainer">
							<div className="mui-scroll">
								<ul className="mui-table-view">
						{this.props.comments.map(function(comment,index){
							return (
									<li key={comment.id} className="mui-table-view-cell comment">
										<div className="avatar">
											<img src={comment.avatar} />
										</div>
										<div className="body">
											<span className="nick">{comment.nick}</span>
											<span className="comment_content" dangerouslySetInnerHTML={{__html: $.replace_em(comment.comment+(comment.to?("<br />@"+comment.to+"://"+comment.to_content):""))}}></span>
											<div className="func">
												<span className="time">{comment.time.split(" ")[0]}</span>
												<i data-cid={comment.id} data-nick={comment.nick} className="reply">回复</i>
											</div>
										</div>
									</li>
							)
					...
					{!this.state.end_flg ?
						(this.state.load_flg ?
						<span className="more">努力加载中...</span>
						:
						<span className="more">点击加载更多</span>
						)
					: null}
					</div>
				:null}
				...
				:null}
					<Share title={this.state.title} content={this.props.content} content_text={this.state.content_txt} link={share.link+"post/"+this.state.companyid+"/"+this.state.postid} thumb={this.state.thumb} ref="share" />
				</div>
			)
		}
	}
