...
define(['react', "react-redux","components/header",
	...
	class Manager extends React.Component {
		...
		render() {
			return ( 
				<div id="manager" className="manager mui-page">
					...
					<div className="mui-page-content">
						<div className="mui-scroll-wrapper">
							...
							{this.props.is_admin == 1? 
								<ul className="management mui-table-view mui-table-view-chevron">
									<li className="mui-table-view-cell">
										<a href="" className="mui-navigate-right mui-icon mui-icon-extra mui-icon-extra-express">全局设置</a>
									</li>
									<li className="mui-table-view-cell">
										<a href="" className="mui-navigate-right mui-icon mui-icon-upload">管理员转让</a>
									</li>
								</ul>
							:null}
								<ul className="management mui-table-view mui-table-view-chevron">
							{this.props.models.map(function(item, index){
								...
								return (
									<li data-id={item.id} data-role={item.leaderid == this.props.uid ? 1:0} className="mui-table-view-cell">
										<a href={href} className={"mui-navigate-right mui-icon "+icon}>{item.name}</a>
									</li>
								)
							}.bind(this))}
								</ul>
