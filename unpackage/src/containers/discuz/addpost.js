		...
		render() {
			return ( 
				...
				{!this.state.keyboard_flg?
					<Header>
						<a className="mui-icon mui-action-back mui-pull-left">关闭</a>
						<h1 className="mui-title">发帖</h1>
						<button type="button" className="addpost-triggler mui-btn mui-btn-primary  mui-pull-right">发表</button>
					</Header>
				:null}
				{!this.state.keyboard_flg || this.state.title_flg?
					<input ref="title" onChange={this.handleChange.bind(this)} onFocus={this.handleFocus.bind(this)} style={this.state.keyboard_flg?{marginTop:"10px"}:{}} onBlur={this.handleBlur.bind(this)} type="text" value={this.state.title} className="post_title" placeholder="一句话描述一下帖子内容" />
				:null}	
					<div className="editarea">
						<div className="toolbar" style={this.state.keyboard_flg && !this.state.title_flg?{position: "fixed",top:"0",zIndex:"3"}:{}} />
						<div className="editor" style={this.state.keyboard_flg && !this.state.title_flg?{marginTop:"32px",height:(plus.android.invoke(plus.android.currentWebview(),"getHeight")/plus.screen.scale-35)+"px"}:{}}>
						</div>
					</div>
				</div>
			)
