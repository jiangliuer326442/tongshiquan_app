	...
	class Putcode extends React.Component {
		...
		rendercodes(){
			...
			for( var i = 0; i < this.state.codelength; i++ ){
				codes.push(React.createElement("input", {type: "number", onChange: this.handleChange.bind(this, i), length: "1"}));
			}
			return codes;
		}
		...
		render(){
			return ( 
			 	React.createElement("div", {className: "reglogin putcode"}, 
		 			React.createElement("div", {className: "head"}, 
		 				React.createElement("img", {onClick: function(){mui.back()}, className: "return", src: "./js/containers/register/img/return.png"})
		 			), 
			 		React.createElement("h2", {className: "title"}, "输入验证码"), 
			 		React.createElement("p", {className: "subtitle"}, "验证码已经发送到", React.createElement("i", null, this.replacephoneno(this.props.phone))), 
			 		React.createElement("div", {className: "codesgroup"}, 
			 			this.rendercodes()
			 		), 
			 		React.createElement("button", {ref: "regbtn", className: this.state.codes.length==this.state.codelength?"btn":"btn btngray", type: "mutton"}, "完成注册")
			 	)
			)
		}
	}
